...
export default function FeedPage() {
  ...
  const handlePostDeleted = (id: string) => setPosts(prev => prev.filter((p: any) => p._id !== id));
  ...
  return (
    <div style={{ backgroundColor: '#fafafa', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        {/* Feed column */}
        <div style={{ width: '100%', maxWidth: isMobile ? '100%' : '470px', flexShrink: 0 }}>
          <StoriesBar />

          {/* Empty state */}
          {posts.length === 0 && !loading && (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#8e8e8e', backgroundColor: 'white', border: isMobile ? 'none' : '1px solid #dbdbdb', borderRadius: '3px', margin: isMobile ? '0 16px' : '0' }}>
              ...
              <p style={{ fontSize: '14px', lineHeight: '20px' }}>When you share photos, they will appear here.</p>
            </div>
          )}

          {/* Posts */}
          <div style={{ margin: isMobile ? '0' : '0' }}>
            {posts.map(post => (
              <PostCard
                key={post._id}
                post={post}
