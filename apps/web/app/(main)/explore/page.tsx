...
export default function ExplorePage() {
  ...
  return (
    <div style={{ backgroundColor: '#fafafa', minHeight: '100vh' }}>
      <Sidebar />
      <div style={{ marginLeft: isMobile ? '0' : '245px', paddingBottom: '80px' }}>
        ...
        {posts.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '80px 20px', color: '#8e8e8e' }}>
            ...
            <p style={{ fontSize: '14px' }}>No posts to explore yet.</p>
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          {groups.map((group, gi) => (
            <div key={gi} style={{ display: 'grid', gridTemplateColumns: gi % 2 === 0 ? '1fr 2fr 1fr' : '2fr 1fr 1fr', gap: '3px' }}>
              {group.map((post, pi) => {
                const isTall = (gi % 2 === 0 && pi === 1) || (gi % 2 === 1 && pi === 0);
                return (
                  <Link key={post._id} href={`/post/${post._id}`}>
                    <div
                      style={{ position: 'relative', paddingBottom: isTall ? '200%' : '100%', backgroundColor: '#dbdbdb', overflow: 'hidden', cursor: 'pointer' }}
                    ...
                    >
                      <img src={post.imageUrl} alt="explore" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
                      <div className="ov" style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '20px', opacity: 0, transition: 'opacity 0.2s' }}>
